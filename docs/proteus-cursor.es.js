...
var e = class e {
	...
	setShape__fluid__animateCursor__calcVelocity() {
		this.addEventListenerTracked(document, "mousemove", (e) => {
			if (this.isDestroyed) return;
			let t = e.clientX - this.prevMouseX, n = e.clientY - this.prevMouseY;
			this.velocity = Math.sqrt(t * t + n * n), this.prevMouseX = this.mouseX, this.prevMouseY = this.mouseY, this.mouseX = e.clientX, this.mouseY = e.clientY;
		});
	}
	setShape__fluid__animateCursor() {
		...
		let e = Math.min(this.velocity / this.maxVelocity, 1);
		if (e > .01) {
			let t = this.mouseX - this.cursorX, n = this.mouseY - this.cursorY, r = Math.sqrt(t * t + n * n);
			if (r > 0) {
				let i = t / r, a = n / r, o = 1 + e * 1.5, s = 1 - e * .3, c = i * i * (o - 1) + 1, l = i * a * (o - 1), u = i * a * (o - 1), d = a * a * (o - 1) + 1, f = -a, p = i, m = c + f * f * (s - 1), h = l + f * p * (s - 1), g = u + f * p * (s - 1), _ = d + p * p * (s - 1);
				this.$shape && (this.$shape.style.transform = `matrix(${m}, ${h}, ${g}, ${_}, 0, 0)`);
			} else this.$shape && (this.$shape.style.transform = "matrix(1, 0, 0, 1, 0, 0)");
		} else this.$shape && (this.$shape.style.transform = "matrix(1, 0, 0, 1, 0, 0)");
		this.$shape && (this.$shape.style.left = this.cursorX - this.$shape.offsetWidth / 2 + "px", this.$shape.style.top = this.cursorY - this.$shape.offsetHeight / 2 + "px"), this.velocity *= .95, this.requestAnimationFrameTracked(this.boundAnimateFluid);
	}
	animateFluidCursor() {
	...
	addState(e, t = {}) {
		return this.isTouch ? this : (this.states[e] = t, this._bindStateElements(e), this);
	}
	removeState(e) {
		return this.isTouch || delete this.states[e], this;
	}
	_applyState(e) {
		let t = this.states[e];
